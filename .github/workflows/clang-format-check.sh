...
# Read ignore patterns (glob style)
...
find "$CHECK_PATH" -name .git -prune -o -regextype posix-egrep -regex "$INCLUDE_REGEX" -print0 |
while IFS= read -r -d '' file; do
  file="${file#./}"   # normalize path

  skip=false

  # check .clang-format-ignore (glob)
  ...
  # apply filters
  if ! $skip && ! [[ ${file} =~ $EXCLUDE_REGEX ]]; then
    format_diff "$file"
  fi
done
